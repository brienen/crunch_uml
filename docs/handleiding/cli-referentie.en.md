...
|---|---|---|
| `detect` | `-f` file | One JSON line with `verdict`, `format` (`eaxmi`, `qea`, `artifact`, `unknown`), `accepted` and `code`. Reads at most 64 KiB (plus the last 64 KiB of an XML file). Exit code 0 = accepted, 2 = refused. |

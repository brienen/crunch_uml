...
class Schema:
    def __init__(self, database, schema_name=const.DEFAULT_SCHEMA):
        if not database:
            raise CrunchException(
                f"Cannot create schema {schema_name} with database with None value."
            )
    ...
    def add(self, obj, recursive=False, processed_objects=set()):
        self.save(
            obj, recursive=recursive, processed_objects=processed_objects, add=True
        )
    ...
    def count_package(self):
        return (
            self.database.session.query(db.Package)
            .filter_by(schema_id=self.schema_id)
            .count()
        )
    ...
    def get_package(self, id):
        return (
            self.database.session.query(db.Package)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )

    def get_diagram(self, id):
        return (
            self.database.session.query(db.Diagram)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_class(self, id):
        return (
            self.database.session.query(db.Class)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_enumeration(self, id):
        return (
            self.database.session.query(db.Enumeratie)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_enumeration_literal(self, id):
        return (
            self.database.session.query(db.EnumerationLiteral)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_attribute(self, id):
        return (
            self.database.session.query(db.Attribute)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_association(self, id):
        return (
            self.database.session.query(db.Association)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def get_generalization(self, id):
        return (
            self.database.session.query(db.Generalization)
            .filter_by(id=id, schema_id=self.schema_id)
            .first()
        )
    ...
    def clean(self):
        self.database.session.query(db.Package).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Class).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Enumeratie).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.EnumerationLiteral).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Attribute).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Association).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Generalization).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.Diagram).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.DiagramAssociation).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.DiagramClass).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.DiagramEnumeration).filter_by(
            schema_id=self.schema_id
        ).delete()
        self.database.session.query(db.DiagramGeneralization).filter_by(
            schema_id=self.schema_id
        ).delete()

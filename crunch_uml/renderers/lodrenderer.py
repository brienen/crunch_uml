...
class LodRenderer(ModelRenderer):
    ...
    This renderer utilizes MIM for ontology representation and SHACL for validation.
    """
    ...
    def render(self, args, zchema: sch.Schema):
        """
        Renders the model into RDF format using the provided schema.
        Validates the generated RDF against SHACL shapes to ensure compliance.
        If no namespace is provided, a default is used.
        """
        ...
        for model in models:
            ...
            for cls in model.classes:
                ...
                g.add((ns[cls.id], RDF.type, mim.Objecttype))  # Add stereotype for class
                ...
                g.add((shape_uri, RDF.type, SH.NodeShape))  # Define a SHACL NodeShape
                g.add((shape_uri, SH.targetNode, ns[cls.id]))  # Target the class node

                for attribute in cls.attributes:
                    attribute_bnode = BNode()  # Create a blank node for the attribute
                    datatype = XSD.string if attribute.primitive is None else attribute.primitive
                    g.add((ns[attribute.id], RDF.type, OWL.DatatypeProperty))  # Define as DatatypeProperty
                    g.add((ns[attribute.id], RDFS.domain, ns[cls.id]))  # Set domain to the class
                    g.add((ns[attribute.id], RDFS.label, Literal(attribute.name)))  # Label for the attribute
                    g.add((ns[attribute.id], RDFS.range, datatype))  # Define the range of the attribute
                    ...
                    g.add((ns[attribute.id], RDF.type, mim.Attribuutsoort))  # Add stereotype for attribute

                    # SHACL property structure
                    g.add((shape_uri, SH.property, attribute_bnode))  # Link property to NodeShape
                    g.add((attribute_bnode, SH.path, ns[attribute.id]))  # Define path for SHACL
                    g.add((attribute_bnode, SH.datatype, datatype))  # Define datatype for SHACL
                    g.add((attribute_bnode, SH.minCount, Literal(1)))  # Minimum count for SHACL
                    g.add((attribute_bnode, SH.maxCount, Literal(1)))  # Maximum count for SHACL
            ...
            for cls in model.classes:
                ...
                for subclass in cls.subclasses:
                    ...
                    if subclass.superclass is not None:
                        ...
                        if super_cls is not None and sub_cls is not None:
                            g.add((sub_cls, RDFS.subClassOf, super_cls))  # Define subclass relationship
                ...
                for assoc in cls.uitgaande_associaties:
                    ...
                    if from_cls is not None and to_cls is not None:
                        g.add((ns[assoc.id], RDF.type, OWL.ObjectProperty))  # Define as ObjectProperty
                        g.add((ns[assoc.id], RDFS.domain, from_cls))  # Set domain for association
                        g.add((ns[assoc.id], RDFS.range, to_cls))  # Set range for association
                        g.add((ns[assoc.id], RDFS.label, Literal(assoc.name)))  # Label for the association
...
class TTLRenderer(LodRenderer):
    """
    Renders Linked Data ontology in Turtle format.
    A model package is a package with at least 1 class inside.
    """
...
class RDFRenderer(LodRenderer):
    """
    Renders Linked Data ontology in RDF format.
    A model package is a package with at least 1 class inside.
    """
...
class JSONLDRenderer(LodRenderer):
    """
    Renders Linked Data ontology in JSON-LD format.
    A model package is a package with at least 1 class inside.
    """

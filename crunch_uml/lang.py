...
def translate(
    value, to_language, from_language, max_retries=3, sleep_between_retries=60
):
    """
    ...
    """
    logger.debug(
        f"Translating text '{value}' from language '{from_language}' to '{to_language}'..."
    )
    attempt = 0
    while attempt < max_retries:
        try:
            ...
            try:
                ...
                attempt += 1
                logger.warning(
                    f"Fout opgetreden vertalen van tekst '{value}' bij poging {attempt}: {e}"
                )
                if attempt < max_retries:
                    logger.warning(
                        f"Wacht {sleep_between_retries} seconden voor nieuwe poging om te vertalen..."
                    )
